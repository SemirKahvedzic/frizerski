...
- The worker (`src/worker/index.ts`) now runs **pg-boss** (own `pgboss` schema in the same database), the outbox relay (`relayOutbox`, `FOR UPDATE SKIP LOCKED`, every `OUTBOX_POLL_MS`), the reminder sweep cron and a health endpoint that reports outbox lag. Application code depends only on the `JobQueue` port in `modules/notifications/queue.ts`; the pg-boss binding lives in `src/worker/pgboss-queue.ts` and tests use `InMemoryQueue`.
- `modules/notifications` is split into pure planning (`plan.ts`: recipients × types × channels with skip reasons, unit-tested), a fresh-state loader (`model.ts`), rendering (`render/booking-email.ts` + `render/ics.ts`, next-intl messages per recipient locale, ICS attachment on confirmations and moves) and persistence (`persist.ts`: one `Notification` row per dedupe key, guest manage tokens minted inside the same transaction). `dispatcher.ts` maps outbox events to triggers, `reminders.ts` claims and sends reminders, `log.ts` serves the admin log and in-app feed.
- Delivery is idempotent end to end: outbox rows are published once (singleton key per event), the dispatcher tolerates redelivery through the unique `dedupeKey`, reminders are claimed with a conditional `UPDATE`, and send handlers only act on `QUEUED` rows. Failed sends retry up to 5 times with backoff and then stay visible as `FAILED` with a **Resend** action on the salon's Notifications page.
- Outbox payloads now carry `actorUserId` so staff notices exclude the person who made the change. Email providers: SMTP (Mailpit in dev/CI), Resend (REST, `RESEND_API_KEY`), console and fake. In-app rows (`IN_APP`) are written for account holders and exposed via `GET /me/notifications`; the bell UI ships with push in Phase 11.

### Implementation notes (Phase 11)

- **Web push** is a third channel in the same pipeline: `buildPlan()` emits `PUSH` items for account holders (salon switch `pushNotificationsEnabled`, personal `pushEnabled`, at least one live `PushSubscription`), `persist.ts` stores a compact payload (`title`, `body`, `url`, `tag = booking:{id}`) and the worker's `notification.sendPush` handler fans it out to every live device of the user. 404/410 from the push service marks the subscription `failedAt` so it is never retried; a user without live devices gets a `SKIPPED / recipient.noSubscription` row.
- Providers behind `PushProvider`: `webpush` (VAPID via the `web-push` library), `fake` (tests/CI) and `off`. The public VAPID key is served by `GET /api/v1/push/vapid-public-key` so it never has to be baked into the client bundle. Subscriptions are managed through `GET/POST/DELETE /me/push-subscriptions`; the first subscription flips `NotificationPreference.pushEnabled` on.
- **PWA**: `src/app/manifest.ts` (installable, standalone, icons in `public/icons`), `public/sw.js` (push display, notification click → focus/open the booking URL, network-first navigations with `/offline.html` fallback) and `RegisterServiceWorker` in the root layout. In development the worker is only registered with `?sw=1` to avoid caching stale chunks. Icons are placeholders until the branding pass.
- Client side, the account → Notifications page shows a **"Push on this device"** card (`PushToggle`) that requests permission, subscribes with the VAPID key and registers/unregisters the device. Staff use the same card; the salon notification log shows the channel per row.

### Implementation notes (Phase 12)

- `modules/media` implements §9: `StorageProvider` port with **s3** (R2/AWS/MinIO via `@aws-sdk/client-s3`), **local** (filesystem under `STORAGE_LOCAL_DIR`, served by `GET /api/v1/media/{key}` with immutable caching) and **fake** (tests). `local` is the development default so the stack runs without MinIO; the compose file still ships MinIO (now from `quay.io/minio`, the Docker Hub images were withdrawn).
- `pipeline.ts` is pure: magic-byte validation (`file-type`, SVG rejected), sharp decode with auto-rotate and metadata stripped, WebP variants `thumb 320 / md 800 / lg 1600 / orig ≤2400` and a 16px inline WebP **placeholder** (stored in `Image.placeholder` instead of the blurhash string from the original plan: no extra dependency, works directly as a CSS background). Processing runs in the request; the `Image.status` column keeps the door open for a worker job.
- Tenant safety at the database level: `Salon.logoImageId/coverImageId`, `Employee.avatarImageId` and `Service.imageId` are plain nullable columns backed by **composite foreign keys** `(salon_id, image_id) → images(salon_id, id)` with `ON DELETE SET NULL (image_id)` (Postgres 15+ column list), so a salon can never reference another salon's image and deleting an image clears every slot. `assertSalonImage()` gives the same guarantee with a readable validation error before the database does.
- UI: reusable `ImageField` (hidden id input + upload/replace/remove) inside the salon profile, employee and service forms; a gallery manager (multi-upload, captions, arrow reordering, delete) at `/admin/[salonSlug]/gallery`; `SalonImage` renders variants with `srcset` and the placeholder. The public page shows cover, logo, team avatars and the gallery; the booking wizard shows avatars.
- Deviations from §9: drag-and-drop (`@dnd-kit`) and client-side cropping are replaced by arrow reordering and server-side fit-inside resizing for now; deletion of objects is synchronous best-effort instead of a retrying job.
...
| 10  | Email notifications          | Worker, outbox relay, dispatcher, react-email templates (bs/en), Resend + Mailpit adapters, reminders 24h/1h with idempotency                                                                                                           | Done — Mailpit E2E assertion, pipeline integration test         |
| 11  | Push notifications + PWA     | Service worker, subscription management, WebPush adapter, staff notifications, installable manifest and offline fallback                                                                                                                | Done — plan unit tests, push integration test                   |
| 12  | Image/gallery management     | Storage adapter (s3/local/fake), upload pipeline, gallery manager (multi-upload, captions, reorder), logo/cover/avatar/service images, public gallery                                                                                   | Done — pipeline unit tests, media integration test, gallery E2E |
...
| 17  | Mobile app (Expo)            | React Native/Expo client on the same REST API: Better Auth bearer/Expo sign-in, salon discovery, booking flow, my appointments, FCM/APNs push via `PushSubscription.platform`, store publishing                                         | Booking from the app appears in the salon calendar              |
