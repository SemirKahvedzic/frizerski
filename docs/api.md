...
| GET / POST / DELETE | `/me/push-subscriptions`                   | Live subscriptions of this browser/user; POST body is `PushSubscription.toJSON()` (`endpoint`, `keys.p256dh`, `keys.auth`); DELETE body `{ endpoint }`. Public `GET /push/vapid-public-key` returns `{ enabled, publicKey }`. |
